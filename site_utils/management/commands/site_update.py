...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option('--noinput', action='store_false', dest='interactive',
                    default=True,
                    help=_('Tells Django to NOT prompt the user for input of '
                           'any kind.')),
    )
    ...
    def handle(self, *args, **options):
        ...
        for arg in args:
            ...
            for cmd_spec in site_update_commands[arg]:
                ...
                cmd_name = cmd_opts[0]
                cmd_args = cmd_opts[1]
                cmd_options = dict(verbosity=verbosity,
                ...
                if not all([app_is_installed(x) for x in cmd_opts[3] or ()]):
                    ...
                    continue
                if verbosity >= 2:
                    print 'Running command "%s" with args %r and options' \
                        ' %r' % (cmd_name, cmd_args, cmd_options)
                # Prevent collectstatic from failing when the clear option is
                # specified and the static root doesn't exist.
                if cmd_name == 'collectstatic' and cmd_options.get('clear',
                                                                   False):
